...
void transpose32_shares(uint32_t *x, size_t stride) {
    for (int s = 0; s < NSHARES; s++) {
        transpose32(&x[s * stride]);
    }
}

// bitslice uint32_t array
// expects n to be a multiple  of 32
void transpose32_array(uint32_t *x, uint32_t n) {
    for (int s = 0; s < NSHARES; s++) {
    ...
    if ((n_coeffs & 0x3) == 0) {
        for (d = 0; d < nshares; d++) {
            for (c = 0; c < n_coeffs; c += 4) {
                int16_t xd0 =
                    dense[c * dense_data_stride + d * dense_msk_stride];
                int16_t xd1 =
                ...
                for (b = 0; b < coeffs_size; b++) {
                    ...
                    bitslice[b * bitslice_data_stride +
                             d * bitslice_msk_stride] |= (xd0 & 0x1) << (c + 0);
                    bitslice[b * bitslice_data_stride +
                             d * bitslice_msk_stride] |= (xd1 & 0x1) << (c + 1);
                    bitslice[b * bitslice_data_stride +
                             d * bitslice_msk_stride] |= (xd2 & 0x1) << (c + 2);
                    bitslice[b * bitslice_data_stride +
                             d * bitslice_msk_stride] |= (xd3 & 0x1) << (c + 3);
                    xd0 = xd0 >> 1;
                    xd1 = xd1 >> 1;
                    xd2 = xd2 >> 1;
                    xd3 = xd3 >> 1;
        ...
        for (d = 0; d < nshares; d++) {
            for (c = 0; c < n_coeffs; c++) {
                ...
                for (b = 0; b < coeffs_size; b++) {
                    bitslice[b * bitslice_data_stride +
                             d * bitslice_msk_stride] |= (xd & 0x1) << c;
                    xd = xd >> 1;
                }
            }
        }
...
void masked_bitslice2dense(size_t nshares, size_t n_coeffs, size_t coeffs_size,
    ...
    for (c = 0; c < n_coeffs; c += 2) {
        for (d = 0; d < nshares; d++) {
            ...
            int16_t xd1 = 0;
            for (b = 0; b < coeffs_size; b++) {
                uint32_t y = bitslice[b * bitslice_data_stride +
                                      d * bitslice_msk_stride];
                xd0 |= ((y >> c) & 0x1) << b;
                xd1 |= ((y >> (c + 1)) & 0x1) << b;
            }
    ...
    for (size_t d = 0; d < nshares; d++) {
        ...
        for (size_t i = 0; i < 32; i++) {
            dense[d * dense_msk_stride + i * dense_data_stride] =
                (a[i] >> 0) & ((1 << 16) - 1);
            dense[d * dense_msk_stride + (i + 32) * dense_data_stride] =
                a[i] >> 16;
        }
    }
}
